...
The manifest check verifies that the curated paper configs cover the GRU, LSTM, and Transformer families and can emit compact JSON or Markdown summaries for inspection. JSON output includes `public_boundary` and `manifest_readiness` blocks with public-scope and ready/not-ready aggregate counts; Markdown output includes the same information in `Public Boundary` and `Manifest Readiness` sections.
